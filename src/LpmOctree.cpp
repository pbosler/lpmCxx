...
Tree::Node::Node() : box(Box3d()), maxAspectRatio(1.0), level(-1), coordsContained(std::vector<index_type>()), 
    parent(NULL), kids(std::vector<std::shared_ptr<Treenode>>()) {}

Tree::Tree(const Coords* crds, const scalar_type maxRatio) : 
...
void generateTree(std::shared_ptr<Treenode> node, std::shared_ptr<Coords> crds, const index_type maxCoordsPerNode) {
    ...
    if (node->coordsContained.size() <= maxCoordsPerNode) {
        ...
        for (int i = 0; i < kids.size(); ++i) {
            ...
            //
            if (!kidcoords.empty()) {
                node->kids.push_back(std::shared_ptr<Treenode>(new Treenode(kids[i], node, kidcoords, node->maxAspectRatio)));
            }
        }
